...
impl<'a, T: Observer + PartialEq> Jvm<'a, T> {
    ...
    pub async fn install_jvm(&self, version: String, game_dir: &PathBuf) -> Result<(),LauncherLibError> {
        ...
        let jvms = manifest.get(&platform).ok_or_else(||LauncherLibError::Generic("Failed to get jvm runtime native.".to_string()))?;
                    ...
                    match file.file_type.as_str() {
                        "file" => {
                            ...
                            if let Some(lzma) = &downloads.lzma {
                                ...
                                let created = current_path.metadata()?.created()?.duration_since(std::time::UNIX_EPOCH)?.as_nanos();
                          
                                return Ok(format!("{} /#// {} {}\n",key,result, created));
                            } else {
                                debug!("JVM FILE RAW: {key}");
                                let hash = utils::download_file(&downloads.raw.url, &current_path, &downloads.raw.sha1).await?;
                                let created = current_path.metadata()?.created()?.duration_since(std::time::UNIX_EPOCH)?.as_nanos();
        ...
        ).buffered(50).collect::<Vec<Result<String,LauncherLibError>>>();

        let sha1_contents = files.await.iter().filter_map(|x|{
            match x {
                Ok(value) => {
                    if value.is_empty() {
                        return None;
...
mod tests {
    ...
    async fn test_jvm_install() {
        ...
        if let Err(err) = jvm.install_jvm("java-runtime-gamma".to_string(),&dir).await {
            eprintln!("{}",err);
        }

import { createLazyFileRoute, Link, ErrorComponent, useLocation } from "@tanstack/react-router";
import { Activity, Download, FileImage, Heart, Sparkles, TriangleAlert } from "lucide-react";
	...
	return (
		<div className="flex h-full overflow-hidden">
			<aside className="w-60 overflow-y-scroll h-full pl-4 space-y-4">
				<div className="space-y-2">
					...
					<RadioGroup onValueChange={(e) => {
						setQuery("");
						navigate({
							search: prev => ({ ...prev, facets: prev?.facets?.setProjectType(e), offset: 0, query: "" })
						})
						...
						{modrinthProjectTypes.data
							?.map((item) => (
								<div key={item} className="flex items-center space-x-2">
									...
									<Label htmlFor={`${item}_radio_opt`}>{item.replace(/^\w/, item[0].toUpperCase())}</Label>
								</div>
							))}
					</RadioGroup>
				</div>
				...
				{Object.entries(categoires.data).map(([key, values]) => {
					return Object.entries(values).filter(e => e[0] === search.facets.getProjectType()).map(([_, item]) => (
						<div className="space-y-2" key={key}>
							<TypographyH3>{key.replace(/^\w/, key[0].toUpperCase())}</TypographyH3>
							<ul className="space-y-2">
								{item.map(item => (
									<Selectable key={item.name} name={item.name} checked={search.facets.hasCategory(item.name)} onChange={() => navigate({
										search: (prev) => ({ ...prev, facets: prev?.facets?.toggleCategory(item.name) })
									})} />
								))}
							</ul>
						</div>
				...
				{search.facets.isAnyProject(SHOW_LOADERS_ON) ? (
					<div className="space-y-2">
						...
						<ul className="space-y-2">
							{filterOn(search.facets.getProjectType(), modrinthLoaders.data)
								?.map((item) => (
									<Selectable key={item.name} name={item.name} checked={search.facets.hasLoader(item.name)} onChange={() => navigate({
										search: (prev) => ({ ...prev, facets: prev?.facets?.toggleLoader(item.name) })
									})} />
								))}
						</ul>
					</div>
						...
						<ul className="space-y-2">
							<li className="flex items-center space-x-2">
								<Checkbox onClick={() => navigate({
									search: (prev) => ({ ...prev, facets: prev?.facets?.toggleEnv("client") })
								})} />
								<Label className="inline-flex items-center gap-1">
									<span className="inline-block h-4 w-4">
										<svg data-v-33894821="" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
							...
							<li className="flex items-center space-x-2">
								<Checkbox onCheckedChange={() => navigate({
									search: (prev) => ({ ...prev, facets: prev?.facets?.toggleEnv("server") })
								})} />
								<Label className="inline-flex items-center gap-1">
									<span className="inline-block h-4 w-4">
										<svg data-v-33894821="" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true">
			...
			<div className="w-full h-full overflow-y-scroll flex flex-col">
				<header className="flex px-4 pt-4">
					<search className="flex w-full gap-4 items-center">
						...
						<Select onValueChange={(e) => navigate({
							search: prev => ({ ...prev, index: e as ModrinthSearchParams["index"], offset: 0 })
						...
						<Select onValueChange={(e) => navigate({
							search: prev => ({ ...prev, limit: Number.parseInt(e), offset: 0 })
				...
				<WorkshopPagination hits={results.data?.hits.length ?? 0} offset={results.data?.offset ?? 0} total_hits={results.data?.total_hits ?? 0} limit={results.data?.limit ?? 0} />
				<div className="grid flex-1 grid-flow-row grid-cols-1 gap-4 px-4 sm:grid-cols-3 xl:grid-cols-3  flex-grow">
					{results.isLoading ? (<>
						...
						<Link search={{} as ModrinthSearchParams} key={project.project_id} to="/workshop/project/$id" params={{ id: project.project_id }}>
							<Card className="h-full flex flex-col">
								...
								<CardContent className="flex flex-grow flex-col">
									<div className="col-span-3 flex flex-col gap-2 justify-between h-full">
										<p className="line-clamp-5 text-sm xl:text-base xl:py-2">{project.description}</p>
										<div className="space-y-1">
											<div className="flex flex-wrap gap-1">
												{project.display_categories?.map(cat => (<Badge key={`${project.project_id}_${cat}`}>{cat}</Badge>))}
											</div>
											<div className="flex items-center gap-2 flex-wrap">
												...
												<div className="mt-auto flex items-center" title="Updated">
													...
													<span className="font-bold">{formatRelative(project.date_modified, new Date())}</span>
												</div>
											</div>
										</div>
									</div>

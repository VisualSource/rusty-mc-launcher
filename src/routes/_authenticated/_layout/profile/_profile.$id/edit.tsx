...
import { copy_profile, db, deleteProfile, showInFolder, uninstallContent } from "@/lib/system/commands";
...
const onFormChange = debounce(async (og: MinecraftProfile, profile: MinecraftProfile) => {
  ...
  await Promise.allSettled(cats.map(e => queryClient.invalidateQueries({ queryKey: [CATEGORY_KEY, e.category] })));
}, 500);

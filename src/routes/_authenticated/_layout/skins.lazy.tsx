...
type Action = { type: "SET_SLIM", value: boolean, id: string } |
...
function DisplayItem({ cape, skin, state, onClick }: { onClick: () => void, state: string, skin?: string, cape?: string, }) {
  ...
  return (
    <button type="button" onClick={onClick}>
      <canvas className={cn("border rounded-lg transition-all hover:scale-105 aspect-square", { "bg-accent bg-opacity-50": state === "ACTIVE" })} width={150} height={150} ref={target} />
    </button>
  );
}
...
function MinecraftSkinControl() {
  ...
  const mutation = useMutation({
    onSuccess(data) {
      queryClient.setQueryData(["account", msAccount?.homeAccountId], (old: MinecraftAccount) => ({
        ...old, details: data
      }));
    },
    onError(error) {
    ...
    mutationFn: async () => {
      ...
      if (ogCapes) {
        ...
        if (prev?.id !== nextCape?.id) {
          ...
          if (!nextCape) {
            const request = await http.delete<MinecraftAccount["details"]>("https://api.minecraftservices.com/minecraft/profile/capes/active", {
              headers: {
            ...
            if (!request.ok) {
              throw new Error((request.data as never as { errorMessage: string }).errorMessage, { cause: request });
            }
      ...
      if (ogSkin?.id !== nextSkin?.id || ogSkin?.variant !== nextSkin?.variant) {
        ...
        if (isUpload(nextSkin.url)) {
          ...
          const filePath = decodeURIComponent(nextSkin.url).replace("https://asset.localhost/", "").replace("asset://", "");

          const filename = filePath.split(sep).at(-1) ?? "player.png";

          const fileContent = await readBinaryFile(filePath);
          formData.set("file", new File([fileContent], filename, { type: "image/png" }));
  ...
  useEffect(() => {
    if (target.current && !isLoading && account) {
      const skins = [...structuredClone(account.details.skins), ...DEFAULT_NAMES.map(e => ({
        id: e,
        state: "INACTIVE",
        url: `https://visage.surgeplay.com/processedskin/${e}.png`,
        variant: e !== "X-Steve" ? "SLIM" : "CLASSIC"
      })) as Skin[]];
      const capes = account.details.capes?.length ? structuredClone(account.details.capes) : [];
      ...
      viewer.current = new SkinViewer({
        ...
        width: 300,
        height: 400,
        model: activeSkin?.variant === "CLASSIC" ? "default" : "slim" ?? "auto-detect",
        skin: activeSkin?.url,
        cape: activeCape?.url
      });
      ...
      <div className={cn('flex h-full', { "hidden": !content.isReady })}>
        <div className="flex flex-col justify-center items-center w-full relative">
          ...
          <div className="flex items-center space-x-2 absolute top-0 left-2">
            <Switch checked={content.skins.find(e => e.state === "ACTIVE")?.variant === "SLIM" ?? false} onCheckedChange={(value) => {
              const current = content.skins.find(e => e.state === "ACTIVE");
              if (current) {
                dispatch({ type: "SET_SLIM", value, id: current.id });
                viewer.current?.loadSkin(current.url, { model: value ? "slim" : "default" });
              }
            }} id="slim-arms" />
            <Label htmlFor="slim-arms">Slim Arms</Label>
          </div>
          <div>
            <Button disabled={mutation.isPending} onClick={() => toast.promise(mutation.mutateAsync(), {
              pending: "Updating",
            ...
            <div className='flex flex-col items-center space-y-4'>
              {content.skins.map(e => (
                <DisplayItem onClick={() => {
                  ...
                  viewer.current?.loadSkin(e.url, { model: e.variant === "CLASSIC" ? "default" : "slim" });
                }} skin={e.url} state={e.state} key={e.id} />
              ))}
              ...
              }} type="button" className="w-[125px] h-[125px] border flex items-center justify-center">
                <Plus />
              </button>
            </div>
          </div>
          <div className="space-y-4">
            ...
            <div className='flex flex-col items-center space-y-4'>
              {content.capes.map(e => (
                ...
                }} cape={e.url} state={e.state} key={e.id} />
              ))}
              <button onClick={() => {
                viewer.current?.loadCape(null);
                dispatch({ type: "SET_CAPE", id: "", state: "INACTIVE" });
              }} className={cn("w-[125px] h-[125px] border", { "bg-accent bg-opacity-50": content.capes.every(e => e.state === "INACTIVE") })} type="button">
                No Cape
              </button>
            </div>
          </div>

...
function DownloadSettings() {
  ...
  const onSubmit = async (state: { dir: string }) => {
    ...
    await queryClient.invalidateQueries({ queryKey: [APP_INSTALLED_MC_VERIONS] })
  };
  ...
  return (
    <div className='space-y-6'>
      ...
      <p className="mt-0 text-muted-foreground text-sm">The localy installed versions of minecraft</p>
      <div>
          ...
          <tbody className="divide-y">
            {data.map((version, key) => (
              <tr key={`${version.name}_${key + 1}`}>
                <td className="text-nowrap text-center p-2 border-r">{version.name}</td>
                <td className="text-center p-2 text-sm text-muted-foreground">{version.path}</td>
              </tr>
            ))}
          </tbody>
        </table>

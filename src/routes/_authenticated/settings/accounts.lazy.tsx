          ...
          <ul className="space-y-4">
            {masl.accounts.length ? (
              masl.accounts.map(acc => (
                <li key={acc.homeAccountId} className='flex flex-row items-center justify-between rounded-lg border p-4'>
                  <div className='flex flex-col'>
                    <span className="text-lg">{acc.name}</span>
                    <span className="text-muted-foreground text-sm italic">{acc.username}</span>
                  </div>
                  {account?.homeAccountId === acc.homeAccountId ? (<Badge>Active</Badge>) : (<Badge variant="secondary">Inactive</Badge>)}
                </li>
              ))
            ) : (

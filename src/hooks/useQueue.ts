...
import DownloadManager, { DOWNLOAD_MANAGER_EVENT_CURRENT } from "@/lib/system/downloadManager";
...
function currentSubscription(callback: () => void) {
	...
	return () => {
		DownloadManager.removeEventListener(DOWNLOAD_MANAGER_EVENT_CURRENT, callback);
	}
}

export function useCurrentQueue() {
	return useSyncExternalStore(currentSubscription, () => DownloadManager.getCurrentSnapshot())
}

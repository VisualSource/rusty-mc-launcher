...
const get_version = (lastVersionId: string): [string, Loader, string | null] => {
...
const import_profiles = async () => {
  ...
  try {
    ...
    const data = JSON.parse(content) as { profiles: Record<string, { lastUsed: string, lastVersionId: string, created: string, icon: string, name: string; type: string, }> }
    ...
    for (const [_key, profile] of Object.entries(data.profiles)) {
      let [version, loader, loader_version] = get_version(profile.lastVersionId);

      if (["latest-release", "latest-snapshot"].includes(version)) {
        version = latest_data.latest[version.replace("latest-", "") as keyof typeof latest_data["latest"]];
      }

      const id = crypto.randomUUID();
      profiles.push({
        name: profile.name.length ? profile.name : `${loader}${loader != "vanilla" ? `(${loader_version})` : ""} ${version}`,
        icon: profile.icon.length ? profile.icon : null,
        id,
        java_args: "-Xmx2G -XX:+UnlockExperimentalVMOptions -XX:+UseG1GC -XX:G1NewSizePercent=20 -XX:G1ReservePercent=20 -XX:MaxGCPauseMillis=50 -XX:G1HeapRegionSize=32M",
        loader,
    ...
    await db.execute({
      query: `INSERT INTO profiles ('id','name','icon','date_created','last_played','version','loader','loader_version','java_args','resolution_width','resolution_height','state') VALUES ${Array.from({ length: profiles.length }).fill(0).map(e => "(?,?,?,?,?,?,?,?,?,?,?,?)").join(", ")};`,
      args: profiles.map(e => [
        e.id,
        e.name,
        e.icon,
    ...
    await db.execute({
      query: `INSERT INTO download_queue VALUES ${Array.from({ length: profiles.length }).fill(0).map(_ => '(?,?,?,?,?,?,?,?,?,?)').join(", ")};`,
      args: profiles.map(e => [crypto.randomUUID(), 1, 0, e.name, e.icon, e.id, (new Date()).toISOString(), "Client", JSON.stringify({ version: e.version, loader: e.loader.replace(/^\w/, e.loader[0].toUpperCase()), loader_version: e.loader_version }), "PENDING"]).flat()
    });

    await queryClient.invalidateQueries({ queryKey: [KEY_DOWNLOAD_QUEUE, QueueItemState.PENDING] });

    toast.success("Imported Profiles");
  } catch (error) {
    logger.error(error);
    toast.error("Import profile error!", { data: { error: (error as Error).message } });
  }
};

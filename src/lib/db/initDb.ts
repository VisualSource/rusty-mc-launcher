...
export async function init() {
    ...
    const result = await categories.execute<{ count: number }>(`SELECT COUNT(*) as count FROM ${categories.name} WHERE profile_id NOT NULL;`);

    if (result?.at(0)?.count === 0) {
        console.log("HELLO")
        const ids = await profiles.find({ select: { id: true } });

        await categories.execute(`INSERT INTO %table% (id,group_id,profile_id) VALUES ${ids?.map((item) => {

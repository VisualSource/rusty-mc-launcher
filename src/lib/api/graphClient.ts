...
export type ImageSize = "48x48" | "64x64" | "96x96" | "120x120" | "240x240" | "360x360" | "432x432" | "504x504" | "648x648";
...
export async function getAccountPhoto(client: Client, userId: string) {
    ...
    const etag = metadata["@odata.mediaEtag"].replace("W/", "").replaceAll("\"", "");
    ...
    if (!hasImage || etag !== etagSaved) {
        ...
        if (!hasDir)
            await createDir("images", { dir: BaseDirectory.AppData });

        const imageRaw = await client.api(`/me/photo/$value`).get() as Blob;

        const buffer = await imageRaw.arrayBuffer();

        await writeBinaryFile(imagepath, [...new Uint8Array(buffer)], { dir: BaseDirectory.AppData });

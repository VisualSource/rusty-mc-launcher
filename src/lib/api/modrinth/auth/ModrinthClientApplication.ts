...
import type { AuthenticationResult, ClearCacheRequest, EndSessionPopupRequest, ITokenCache, PopupRequest, RedirectRequest, SsoSilentRequest } from "@masl/index";
import { AuthResponse, PopupClient } from './PopupClient';
import { BrowserAuthErrorCodes, createBrowserAuthError } from "@/lib/masl/error/BrowserAuthError";
...
export class ModrinthClientApplication {
    ...
    public async acquireTokenPopup(): Promise<string> {
        ...
        localStorage.setItem(`modrinth.user.${response.id}`, JSON.stringify(response));
        localStorage.setItem(`modrinth.auth.${response.id}`, JSON.stringify({
    ...
    public async acquireTokenSilent(silentRequest: SilentRequest): Promise<string> {
        let account = silentRequest?.account ? silentRequest.account : this.getActiveAccount();
        if (!account) {
        ...
        if (!cache) throw createBrowserAuthError(BrowserAuthErrorCodes.noTokenRequestCacheError);
    ...
    public getAccountByUsername(userName: string): AccountInfo | null {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            try {
                ...
                auth.error(`Failed to parse user account info: ${(error as Error)?.message}`);
            }
        }
        return null;
    }
    ...
    public loginPopup(request?: PopupRequest | undefined): Promise<AuthenticationResult> {
        throw new Error("Method not implemented.");
    }
    public logoutPopup(logoutRequest?: EndSessionPopupRequest | undefined): Promise<void> {
        throw new Error("Method not implemented.");
    }
    ...
    public hydrateCache(result: AuthenticationResult, request: PopupRequest | RedirectRequest | SilentRequest | SsoSilentRequest): Promise<void> {
        throw new Error("Method not implemented.");
    }
    public clearCache(logoutRequest?: ClearCacheRequest | undefined): Promise<void> {

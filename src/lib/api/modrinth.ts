...
export type ModrinthApiSearchResponse = {
...
const modrinth = {
  Search: async ({
    ...
    const response = await fetch(
      `https://api.modrinth.com/v2/search?${urlParams.join("&")}`,
      {
        headers: {
  ...
  GetPackFile: async (packId: string): Promise<FileDownload> => {
    const response = await fetch(
      `https://api.modrinth.com/v2/project/${packId}/version`,
    );
    if (!response.ok)
      throw new Error("Failed to fetch mod data", {
        cause: "FAILED_TO_REQUEST_DATA",
      });
    ...
    if (!version)
      throw new Error("Failed to find version for wanted version", {
        cause: "NO_VAILD_VERSIONS",
      });
   ...
   * @param game
   * @param fromVersion
   * @returns {Promise<FileDownload[]>} A list of files that need to be download.
   */
  GetVersionFiles: async (
    ...
    const response = await fetch(
      fromVersion
        ...
        : `https://api.modrinth.com/v2/project/${id}/version?loaders=["${loader}"]&game_versions=["${game}"]`,
    );
    ...
    if (!file)
      throw new Error("Failed to get file download", {
        cause: "FAILED_TO_FIND_PRIMARY_DOWNLOAD",
      });
    ...
    const deps = await Promise.all(
      first.dependencies.map(async (item) => {
        return modrinth.GetVersionFiles(
          item.project_id ?? item.version_id,
          loader,
          game,

...
// override
internals.PopupClient.prototype.monitorPopupForHash = async function (
  this: internals.PopupClient,
  popupWindow: Window,
): Promise<string> {
  return new Promise<string>(async (resolve, reject) => {
    ...
    if (!port) {
      return reject(
        BrowserAuthError.createPopupWindowError(
...
internals.PopupClient.prototype.waitForLogoutPopup = async function (
  this: internals.PopupClient,
  popupWindow: Window,
): Promise<void> {
  return new Promise<number>(async (resolve) => {
    ...
    .then((port) => {
      try {
        fetch(`http://127.0.0.1:${port}/exit`);
      } catch (e) {}
    })
    .finally(() => {
      this.cleanPopup(popupWindow);

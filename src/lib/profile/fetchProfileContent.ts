...
export async function fetchProfileContent(profileId: string, contentType: keyof typeof ContentType) {
    const data = await query`SELECT * FROM profile_content WHERE profile = ${profileId} AND type = ${contentType};`
        .as(ContentItem)
        .all();

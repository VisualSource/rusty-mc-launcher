type FacetsValues = { loaders: string[], versions: string[], client_side: string[], server_side: string[], categories: string[], project_type: string }

export class Facets {
    static SHADER_LOADERS = ["canvas", "iris", "optifine", "vanilla shader"]
    static INCLUDE_LOADERS = ["fabric", "neoforge", "forge", "quilt", "modloader"];
    ...
    constructor(facets: string[][]) {
        for (const facet of facets) {
            for (const item of facet) {
                ...
                if (key === "categories" && (Facets.INCLUDE_LOADERS.includes(v) || Facets.SHADER_LOADERS.includes(v))) {
                    this.facets.loaders.push(v);
                    continue;
                }
                const group = this.facets[key as keyof FacetsValues] as Array<string> | undefined;
                if (!group) continue;
                group.push(v);
            }
        }
    ...
    getDisplayProjectType(): string {
        if (this.facets.project_type === "mod" && this.facets.categories.includes("datapack")) {
            return "datapack";
        }
        return this.facets.project_type;
    }
    toggleEnv(name: "client" | "server"): this {
        switch (name) {
            case "client":
                if (this.facets.server_side.length === 2 && this.facets.server_side.includes("required")) {
                ...
                break;
            case "server":
                if (this.facets.client_side.length === 2 && this.facets.client_side.includes("required")) {
                    this.facets.server_side = ["required"];
                    this.facets.client_side = ["required"];
                    break;
                }

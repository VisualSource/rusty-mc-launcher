...
import {
	...
	type StaticAuthorityOptions,
	CacheHelpers,
	type StoreInCache,
	CacheError,
} from "@azure/msal-common";
import type { CacheOptions } from "../config/Configuration";
import {
						...
						switch (credObj["credentialType"]) {
							case CredentialType.ID_TOKEN:
								if (CacheHelpers.isIdTokenEntity(credObj)) {
									...
									const idTokenEntity = credObj as IdTokenEntity;
									const newKey = this.updateCredentialCacheKey(
										key,
										idTokenEntity,
									);
							...
							case CredentialType.ACCESS_TOKEN_WITH_AUTH_SCHEME:
								if (CacheHelpers.isAccessTokenEntity(credObj)) {
									...
									const accessTokenEntity = credObj as AccessTokenEntity;
									const newKey = this.updateCredentialCacheKey(
										key,
										accessTokenEntity,
									);
							...
							case CredentialType.REFRESH_TOKEN:
								if (CacheHelpers.isRefreshTokenEntity(credObj)) {
									...
									const refreshTokenEntity = credObj as RefreshTokenEntity;
									const newKey = this.updateCredentialCacheKey(
										key,
										refreshTokenEntity,
									);

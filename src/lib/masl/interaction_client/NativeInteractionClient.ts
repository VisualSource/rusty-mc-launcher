...
import type { NavigationOptions } from "../navigation/NavigationOptions";
...
export class NativeInteractionClient extends BaseInteractionClient {
	...
	protected async generateAuthenticationResult(
		...
		const uid =
			(accountProperties as Record<string, string>)["UID"] ||
			idTokenClaims.oid ||
			idTokenClaims.sub ||
			Constants.EMPTY_STRING;
		const tid =
			(accountProperties as Record<string, string>)["TenantId"] ||
			idTokenClaims.tid ||
			Constants.EMPTY_STRING;
		...
		const expiresIn: number =
			request.tokenType === AuthenticationScheme.POP
				...
				: (typeof response.expires_in === "string"
						? Number.parseInt(response.expires_in, 10)
	...
	private handleExtraBrokerParams(request: NativeTokenRequest): void {
		...
		if (
			...
			const brk_redirect_uri =
				request.extraParameters[BrokerServerParamKeys.BROKER_REDIRECT_URI];

...
export class PopupClient extends StandardInteractionClient {
    ...
    monitorPopupForHash(popupWindow: Window): Promise<string> {
        ...
        return (async () => {
            ...
            if (!port) throw createBrowserAuthError(BrowserAuthErrorCodes.popupWindowError);
            this.logger.verbose(
            ...
            clearInterval(intervalId);
            if (port !== null) fetch(`http://127.0.0.1:${port}/exit`).catch((e) => this.logger.error(e.message ?? "Close auth server error."));
            this.cleanPopup(popupWindow);
            unlisten?.then(unsubscribe => unsubscribe()).catch(e => this.logger.error(e?.message ?? "Failed to unsubscribe."));
        });
    }

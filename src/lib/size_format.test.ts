...
test("IEC Format 1280 bytes with 1 digit and space units", () => {
    expect(formatSize(1280, { digit: 1, space: true, mode: "iec" })).toBe("1.3 KiB");
});
...
test("IEC Format 1YiB", () => {
    expect(formatSize(BigInt(1208925819614629174706176), { mode: "iec" })).toBe("1YiB");
});
...
test("Text Range Error 2", () => {
    expect(() => formatSize(1208925819614629174706176, { mode: "iec" })).toThrowError();
});
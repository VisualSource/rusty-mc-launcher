...
export class MinecraftSemver {
    static parse(version: string): SnapshotVersion | ReleaseVersion {
        ...
        if (isSnapshot) {
            ...
            if (!data || !data?.groups) throw new Error("Failed to parse snapshot version.");
    ...
    static gt(a: string, b: string): boolean {
        ...
        if (va.type === "release" && vb.type === "release") {
            ...
            if ((va.rc === null && va.pre === null) && (vb.rc !== null || vb.pre !== null)) return true;
        ...
        if (va.type === "snapshot" && vb.type === "snapshot") {
            ...
            if ((va.identifier.length === 1 && vb.identifier.length === 1) && (va.identifier.charCodeAt(0) > vb.identifier.charCodeAt(0))) {
                return true;
            }

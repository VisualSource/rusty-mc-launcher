...
export class MinecraftSemver {
	static parse(version: string): SnapshotVersion | ReleaseVersion {
		...
		if (isSnapshot) {
			...
			return {
				type: "snapshot",
				year: Number.parseInt(data.groups.year),
				week: Number.parseInt(data.groups.week),
		...
		if (candidate) {
			if (candidate.startsWith("rc")) {
				release_candidate = Number.parseInt(candidate.replace("rc", ""));
			} else if (candidate.startsWith("pre")) {
				prerelease = Number.parseInt(candidate.replace("pre", ""));
			}
		}

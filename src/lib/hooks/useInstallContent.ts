...
const useInstallContent = () => {
  ...
  const installContent = useCallback(
    async (
      slug: string,
      type: "resourcepack" | "shader" | "mod" | "modpack",
      opts?: {
        ...
        switch (type) {
          ...
          case "mod": {
            ...
            const version = await selectVersion({
              minecraft_versions: opts.minecraft_versions,
              modloaders: opts.modloaders,
            });
            if (!version) break;
            const profile = await selectProfile({
              minecraft_versions: [version.game],
              modloaders: [version.loader],
            });
            if (!profile) break;
            installMods(profile, [slug], version);
            break;
          }
          case "modpack":

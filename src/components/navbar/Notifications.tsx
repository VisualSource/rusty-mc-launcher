...
export const Notifications = () => {
    const { notifications, markAllAsRead, remove, clear } = useNotificationCenter();
    return (

        <Popover>
            <PopoverTrigger asChild>
                <button type="button" className={cn("flex items-center px-2.5 mr-2 transition-colors text-white", { "bg-green-500": notifications.length > 0, "bg-zinc-800": notifications.length === 0 })}>
                    {notifications.length > 0 ? <span className="text-sm mr-2">{notifications.length}</span> : null}

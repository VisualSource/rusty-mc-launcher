...
export const DownloadSettings: React.FC = () => {
    ...
    const onSubmit = async (state: { dir: string }) => {
        if (!await exists(state.dir)) {
            form.setError("dir", { type: "validate", message: "The new path does not exist." });
            return;
        }
        ...
        <>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-2">
                    ...
                    <FormField rules={{ required: { message: "The app directory is required", value: true } }} control={form.control} name="dir" render={({ field }) => (
                        <FormItem>
                            ...
                            <FormDescription>
                                The directory where all file are put. Ex. profile, and game runtime files.
                            </FormDescription>
                            <FormMessage />
                        </FormItem>
                    )} />

...
export const LaunchSettings: React.FC = () => {
    const form = useForm<{ exit_on_start: boolean }>({
        async defaultValues() {
            try {
                const exit_on_start = await settings.get_setting("option.exit_on_start")
    ...
    const onSubmit = async (state: { exit_on_start: boolean }) => {
        const [affected] = await settings.update("option.exit_on_start", state.exit_on_start ? "TRUE" : "FALSE");
        if (affected === 0) {
            await settings.insert("option.exit_on_start", state.exit_on_start ? "TRUE" : "FALSE");
        }
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <div className="flex items-center gap-2 flex-row-reverse justify-end">
                                    ...
                                    <FormControl>
                                        <Switch checked={field.value} onCheckedChange={(ev) => field.onChange(ev)} />
                                    </FormControl>
                                </div>
                                <FormDescription>

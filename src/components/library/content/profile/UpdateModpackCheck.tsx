...
export const UpdateModpackCheck: React.FC<{
	...
	const { data } = useQuery({
		...
		queryFn: async () => {
			if (!profile.is_modpack)
				throw new Error("No modpack metadata was provided");
			...
			switch (content.version_type) {
				...
				case "modrinth_id": {
					...
					if (!version || !version?.version_number)
						throw new Error("Failed to get lastest version for modpack");

					const hasUpdate =
						compare(content.version, version.version_number) === -1;
					return {
						hasUpdate: hasUpdate,
						nextVersionData: hasUpdate ? version : null,
					};
				}
				default:
					throw new Error(
						`Unknown modpack metadat type: Given ${content.version_type}`,
					);
			}
		},
	});
	...
	return (
		<Tooltip>
			<TooltipTrigger asChild>
				<div className="relative">
					...
					<Button
						...
						onClick={() => {
							if (!data.hasUpdate) {
								message("Your modpack is all up to date", {
							...
							ask(
								`Version ${data.nextVersionData?.version_number} is available. Do you want to update?`,
								{ title: "Modpack Updater", kind: "info" },
							)
								.then(async (shouldUpdate) => {
									if (!shouldUpdate) return;

									const file = data.nextVersionData?.files.find(
										(e) => e.primary,
									);
									if (!file)
										throw new Error("Failed to get primary file!", {
											cause: data.nextVersionData,
										});
								...
								.catch((e) => {
									toastError({
										...
										description: "Failed to update modpack",
									});
									console.error(e);
								});
						}}
			...
			<TooltipContent>
				<p>
					{data.hasUpdate
						? `Modpack ${data.nextVersionData?.version_number} is available!`
						: "Modpack Update"}
				</p>
			</TooltipContent>
		</Tooltip>

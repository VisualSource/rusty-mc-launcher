...
const CategorySelectCore: React.FC<{ profile: string }> = ({ profile }) => {
    ...
    const { data, isError, error, isLoading } = useQuery({
        ...
        queryFn: () => db.select({
            ...
            args: [profile], schema: categories.schema
        })
    });
        ...
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button variant="secondary" className="w-full justify-start gap-2" type="button">
                    {data?.length ? (<>
                        {data.map((item) => (<Badge key={item.id}>{categoryList.find(e => e.metadata === item.category)?.value}</Badge>))}
                    </>) : (<>
                        Select Categories
                    </>)}
                </Button>
                        ...
                        <CommandGroup>
                            {categoryList.map((category) => (
                                <CommandItem value={category.metadata!} onSelect={(currentValue) => {
                                    const action = data?.findIndex(e => e.category === currentValue) === -1 ? "add" : "remove";
                                    ...
                                    <Check className={cn("mr-2 h-4 w-4", data?.findIndex(e => e.category === category.metadata) === -1 ? "opacity-0" : "opacity-100")} />
                                    {category.value}
                                </CommandItem>
                            ))}
                        </CommandGroup>

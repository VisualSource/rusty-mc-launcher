...
export const LoaderVersionSelector: React.FC<{ form: UseFormReturn<MinecraftProfile, any, undefined>; stable: boolean }> = ({ stable, form }) => {
        ...
        queryFn: async () => {
            switch (loader) {
                ...
                case "quilt": {
                    ...
                    return data.filter(item => !item.version.includes("beta")).map(e => e.version);
                }
                case "forge": {
    ...
    return (
        <FormField
            ...
            name="loader_version"
            rules={{ required: { value: true, message: "A loader version is required!" } }}
            render={({ field }) => (
                <FormItem>
                    ...
                    <FormControl>
                        <Popover open={open} onOpenChange={setOpen}>
                            <PopoverTrigger asChild>
                                <Button className="w-full" disabled={isError} variant="outline" role="combobox" type="button" aria-expanded={open}>
                            ...
                            <PopoverContent className="w-[200px] p-0">
                                <Command>
                                    ...
                                    {isLoading ? (<CommandLoading>Loading versions...</CommandLoading>) : null}
                                    <CommandEmpty>No versions found.</CommandEmpty>
                                    <CommandList className="scrollbar">
                                        {data?.map((version) => (
                                            <CommandItem key={version} value={version} onSelect={(currentValue) => {
                                                field.onChange(field.value === currentValue ? undefined : currentValue);
                                                setOpen(false);
                                            }}>
                                                <Check className={cn("mr-2 h-4 w-4", field.value === version ? "opacity-100" : "opacity-0")} />
                                                {version}
                                            </CommandItem>
                                        ))}

...
import { categoryList, loaderList, projectTypeList } from "@/lib/api/modrinth/services.gen";
...
export const SearchFilters: React.FC<{ search: ModrinthSearchParams, navigate: UseNavigateResult<string> }> = ({ navigate, search }) => {
    const categoires = useSuspenseQuery({
        ...
        queryFn: async () => {
            ...
            if (!response.ok || !data) throw new Error("Failed to load categories", { cause: response });
    ...
    const modrinthProjectTypes = useSuspenseQuery({
        ...
        queryFn: async () => {
            ...
            if (error) throw error;
            if (!response.ok || !data) throw new Error("Failed to load project types", { cause: response });

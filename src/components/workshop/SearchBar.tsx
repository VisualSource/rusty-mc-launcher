...
const CheckBoxItem: React.FC<{ label: string, id: string; icon?: string }> = ({ label, id, icon }) => {
    return (
        <div className="flex items-center space-x-2">
            ...
            >
                {icon ? <span className="h-4 w-4 inline-block" dangerouslySetInnerHTML={{ __html: icon }}></span> : null}{label.replace(/^./, label[0].toUpperCase() ?? "")}
            </label>
        </div>
    );
}
...
const SearchBar = () => {
    ...
    return (
        <section className="flex justify-center shadow-xl">
            <search className="w-2/3 p-2 flex gap-2">
                ...
                <Popover>
                    <PopoverTrigger asChild>
                        <Button><Filter className="h-5 w-5 mr-2" /> Filter</Button>
                    </PopoverTrigger>
                        ...
                        <ScrollArea className="h-96">
                            <div className="space-y-2">
                                ...
                                <ul className="space-y-2">
                                    {categoires.data?.filter((e) => e.project_type === "mod")?.map((item) => (
                                        <li key={item.name}>
                                            <CheckBoxItem label={item.name} icon={item.icon} id={item.name} />
                                        </li>
                                    ))}
                                </ul>
                            </div>
                            <div className="space-y-2">
                                ...
                                <ul className="space-y-2">
                                    {loaders.data?.filter(e => e.supported_project_types.includes("mod") && e.supported_project_types.includes("modpack")).map((e) => (
                                        <li key={e.name}>
                                            <CheckBoxItem label={e.name} icon={e.icon} id={e.name} />
                                        </li>
                                    ))}
                                </ul>
                            </div>

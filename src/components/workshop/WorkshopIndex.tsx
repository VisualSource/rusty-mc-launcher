    ...
    return (
        <div className="h-full flex flex-col gap-2">
            <form className="grid grid-cols-12 text-zinc-50 py-2 px-4 gap-2 row-span-1">
                <Input onChange={(ev) => setSearch(ev.target.value)} value={search} className="col-span-6" placeholder="Search" />
                <Select defaultValue="project_type:mod" onValueChange={(ev) => {
                    ...
                    <SelectContent>
                        ...
                        <SelectItem value="project_type:resourcepack">View: Resourcepacks</SelectItem>
                        <SelectItem value="project_type:shader">View: Shaderpacks</SelectItem>
                    </SelectContent>
                </Select>
                ...
                {data.hits.map((project) => (
                    <Link key={project.project_id} to={`/workshop/${project.project_id}`}>
                        <Card className="h-full">
                            <CardHeader>
                                <CardTitle>{project.title} - {project.author}</CardTitle>
                                <div className="flex flex-wrap">
                                    {project.display_categories.map((value, i) => (<Badge key={i}>{value}</Badge>))}
                                </div>
                            </CardHeader>

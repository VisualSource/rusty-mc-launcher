...
const SelectProfile: React.FC = () => {
  ...
  const avaliable = useMemo(() => {
    ...
    return profiles.filter((item) => {
      ...
      const loader = config.modloaders
        ? config.modloaders.includes(type)
        : true;
  ...
  const onSubmit = (ev: z.infer<typeof FormSchema>) => {
    window.dispatchEvent(
      new CustomEvent(RETURN_KEY, { detail: { id: ev.profile ?? null } }),
    );
    setOpen(false);
  };
  ...
  return (
    <Dialog
      ...
      onOpenChange={(state) => {
        ...
        if (!state) {
          window.dispatchEvent(
            new CustomEvent(RETURN_KEY, { detail: { id: null } }),
          );
        }
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4 shadow">
                        ...
                        <div className="space-y-1 leading-none">
                          <FormLabel>Show all profiles.</FormLabel>
                          <FormDescription>

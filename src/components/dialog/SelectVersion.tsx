...
export const selectVersion = async (target: {
  modloaders: string[];
  minecraft_versions: string[];
}): Promise<{ game: string; loader: string } | null> => {
  return new Promise((resolve) => {
    window.addEventListener(
      RETURN_KEY,
      (ev) => {
        const data = (
          ev as CustomEvent<{ game: string; loader: string } | null>
        ).detail;
        resolve(data);
      },
      { once: true },
                      ...
                      <Popover onOpenChange={setMCPopover} open={mcPopover}>
                        <PopoverTrigger asChild>
                          <FormControl>
                            ...
                            >
                              {field.value
                                ? `Minecraft ${config.minecraft_versions.find(

...
const GameCrash: React.FC = () => {
    ...
    useEffect(() => {
        const handler = (ev: Event) => {
            const data = (ev as CustomEvent<{ exitCode: number, msg?: string; }>).detail;
    ...
    return (
        <AlertDialog open={open} onOpenChange={setOpen}>
            <AlertDialogContent className="text-zinc-50">
                <AlertDialogHeader>
                    <AlertDialogTitle>{!exitMessage ? "Game Crash" : "Failed To Start"}</AlertDialogTitle>
                    <AlertDialogDescription>{!exitMessage ? `The game did not exit successfully. (Exit Code: ${exitCode})` : `Error starting game: ${exitMessage}`}</AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>

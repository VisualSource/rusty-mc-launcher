...
export const FabricLoaderVersionSelector: React.FC<{ defaultValue?: string, onChange?: (value: string) => void }> = ({ onChange, defaultValue }) => {
        ...
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button disabled={isLoading} aria-expanded={open} type="button" role="combobox" variant="outline">
                    {isLoading ? "Loading Versions..." : value ? `Fabric Loader ${data?.find(version => version.version === value)?.version}` : "Select Version"}
            ...
            <PopoverContent className="w-[200px] p-0">
                <Command>
                    ...
                    <CommandList className="scrollbar max-h-48">
                        ...
                        <CommandGroup>
                            {data?.map((item) => (
                                <CommandItem className="z-[10000000] w-full" key={item.version} value={item.version} onSelect={(currentValue) => {

...
const sizeOptions = z.object({
	...
	digit: z
		.number()
		.gte(0, "`digits` must be an integer greater than or equal to 0.")
		.optional()
		.default(0),
});
...
const numCheck = z
	...
	.finite()
	.refine(
		(arg) => Number.isSafeInteger(arg),
		`\`BigInt\` should be used when specifying huge numbers (Value greater than ${Number.MAX_SAFE_INTEGER}).`,
	);
const bigintCheck = z
	.bigint()
	.gte(BigInt(0), "The file size must be a number greater than or equal to 0.");
...
export function formatSize(
	...
	if (typeof size === "number") {
		...
		for (const [exponentStr, unit] of Object.entries(
			CONFIG[optionsInited.mode].UNIT_TABLE,
		)) {

...
export const uploadSkin = async (
	skin: { url: string; variant: Skin["variant"] },
	accessToken: string,
) => {
	...
	if ("error" in data) {
		throw new Error(
			`Failed to set minecraft skin: ${data.errorMessage ?? data.error}`,
			{ cause: data },
		);
	}
...
export const setCape = async (accessToken: string, capeId?: string) => {
	...
	if (!response.ok)
		throw new Error("Failed to update cape", { cause: response.statusText });

	const data = (await response.json()) as ApiResponse | ApiErrorResponse;
	if ("error" in data)
		throw new Error(
			`There was an error setting the cape: ${data.errorMessage ?? data.error}`,
			{ cause: data },
		);
	return data;
};
